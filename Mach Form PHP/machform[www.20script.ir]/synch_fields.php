 ...
 ********************************************************************************/
	...
	if(!empty($element_properties_array)){
		foreach($element_properties_array as $element_properties){
			...
			$element_options = $element_properties['options'] ?? false;
			...
			if(in_array($element_properties['type'],array('radio','checkbox','select'))){
				...
				$query = "INSERT INTO 
							  ...
							  VALUES 
							  		(:form_id,:element_id,:option_id,:position,:option,:is_default,:is_hidden,'2');"; 
				...
				foreach ($element_options as $m_element_id=>$value){
					...
					$query = "INSERT INTO 
										...
										(`form_id`,`element_id`,`option_id`,`position`,`option`,`option_is_default`,`option_is_hidden`,`live`) 
								  VALUES 
								   		(:form_id,:element_id,:option_id,:position,:option,'0','0','2');";
