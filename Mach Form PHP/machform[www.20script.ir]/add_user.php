 ...
 ********************************************************************************/	
	...
	if(mf_is_form_submitted()){ //if form submitted
		...
		$user_input['send_login'] 		= isset($_POST['au_send_login']) ? (int) $_POST['au_send_login'] : false;

		$user_input['priv_new_forms'] 	= isset($_POST['au_priv_new_forms']) ? (int) $_POST['au_priv_new_forms'] : false;
		$user_input['priv_new_themes'] 	= isset($_POST['au_priv_new_themes']) ? (int) $_POST['au_priv_new_themes'] : false;
		$user_input['priv_administer'] 	= isset($_POST['au_priv_administer']) ? (int) $_POST['au_priv_administer'] : false;
		...
		foreach ($form_list_array as $value) {
			...
			$user_input['perm_editform_'.$form_id] 	  = isset($_POST['perm_editform_'.$form_id]) ? (int) $_POST['perm_editform_'.$form_id] : 0;
			$user_input['perm_editreport_'.$form_id]  = isset($_POST['perm_editreport_'.$form_id]) ? (int) $_POST['perm_editreport_'.$form_id] : 0;
			$user_input['perm_editentries_'.$form_id] = isset($_POST['perm_editentries_'.$form_id]) ? (int) $_POST['perm_editentries_'.$form_id] : 0;
			$user_input['perm_viewentries_'.$form_id] = isset($_POST['perm_viewentries_'.$form_id]) ? (int) $_POST['perm_viewentries_'.$form_id] : 0;
		}
		//clean the inputs
				...
				<div class="content_header">
					<div class="content_header_title">
						<div style="float: left">
							<h2><a class="breadcrumb" href='manage_users.php'>Users Manager</a> <span class="icon-arrow-right2 breadcrumb_arrow"></span> Add User</h2>
						...
						<div style="float: right;margin-right: 0px;padding-top: 26px;">
								<a href="add_user_bulk.php" id="add_user_bulk_link" class="">
									<span class="icon-tab"></span> Switch to <strong>Bulk Add Users</strong> Mode
								</a>
						</div>	
						...
						<li>
							<div id="au_box_user_profile" class="au_box_main gradient_blue">
								...
								<div class="au_box_content" style="padding-bottom: 15px">
									...
									<input id="au_user_name" name="au_user_name" class="element text large" value="<?php echo isset($user_input['user_name']) ? htmlspecialchars($user_input['user_name']) : ''; ?>" type="text">
									<?php
									...
									<label class="inline description <?php if(!empty($error_messages['user_email'])){ echo 'label_red'; } ?>" for="au_user_email">Email Address </label>
									...
									<input id="au_user_email" name="au_user_email" class="element text large" value="<?php echo isset($user_input['user_email']) ? htmlspecialchars($user_input['user_email']) : ''; ?>" type="text">
									<?php
									...
									<input id="au_user_password" name="au_user_password" class="element text medium" value="<?php echo isset($user_input['user_password']) ? htmlspecialchars($user_input['user_password']) : ''; ?>" type="password" style="width: 60%">
									<?php
									...
									<span style="display: block;margin-top: 15px">	
										...
										<label class="choice inline" for="au_send_login">Send login information to user</label> 
										<span class="icon-question helpicon clearfix" data-tippy-content="Checking this option will send an email to the user. The email contains their password and the login link to MachForm admin panel."></span>
									</span>
									...
									<ul id="au_li_permissions">
										...
										?>
												<li id="li_<?php echo $form_id; ?>" <?php echo $class_attr; ?>>
													<div class="au_perm_title"><span class="icon-document"></span> <?php echo $value['form_name']; ?></div>
