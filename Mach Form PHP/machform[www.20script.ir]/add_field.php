<?php
	...
	/* 1. Get new field parameters 																	   			   */
	/***************************************************************************************************************/
	$element_type  	 			= isset($_POST['element_type']) ? strtolower(trim($_POST['element_type'])) : false;
	$form_id					= isset($_POST['form_id']) ? (int) $_POST['form_id'] : 0;
	$element_position			= isset($_POST['position']) ? (int) $_POST['position'] : 0; //the position of the element within the preview page
	$element_properties_input 	= isset($_POST['field_properties']) ? mf_sanitize($_POST['field_properties']) : false;
	...
	switch ($element_type) {
			...
			case 'media' 		: $element_title = 'Media';break;
			case 'rating' 		: $element_title = 'Rating';break;
	}
	...
	if(($action == 'click_new') || ($action == 'drag_new')){
		...
		$element_properties['date_disable_dayofweek'] = 0;
		$element_properties['date_disabled_dayofweek_list'] = '';
		$element_properties['date_disable_specific'] = 0;
		$element_properties['date_disabled_list'] = '';
		$element_properties['file_type_list'] = 'jpg, jpeg, png, gif, bmp, heic, pdf, docx, doc, xlsx, xls, ppt, pptx, txt, csv, zip, mp3, wma, mpg, mpeg, mp4, avi'; //the default is to block all file types except this list
		...
		$element_properties['media_type'] = 'image'; //possible values: image, video, html
		$element_properties['media_image_alignment'] = 'l'; //possible values: l,c,r (left, center, right) 
		$element_properties['media_video_size']	= 'large';
		$element_properties['media_display_in_email'] = 0;
		$element_properties['rating_style'] = 'star'; //possible values: star,circle,love,thumb  
		$element_properties['rating_max'] = 5;
		$element_properties['rating_default'] = 0;
		$element_properties['rating_enable_label'] = 0;
		$element_properties['rating_label_high'] = 'Best';
		$element_properties['rating_label_low'] = 'Worst';
		$element_properties['address_subfields_labels'] = '';
		$element_properties['address_subfields_visibility'] = '';
		...
		//default value for signature field
		if($element_type == 'signature'){
			$element_properties['guidelines'] 	= 'I understand this is a legal representation of my signature.';
		}
	...
	if(in_array($element_type,array('radio','checkbox','select','matrix'))){
		if(($action == 'click_new') || ($action == 'drag_new')){ //if this is creating new field
			...
			if(in_array($element_type,array('radio','checkbox','select'))){
				$query = "INSERT INTO 
									 ".MF_TABLE_PREFIX."element_options (`form_id`,`element_id`,`option_id`,`position`,`option`,`option_is_default`,`option_is_hidden`,`live`)
						  	   SELECT 
						  	   		 `form_id`, '{$element_id}' as new_element_id,`option_id`,`position`,`option`,`option_is_default`,`option_is_hidden`,'2' as `live` 
						  	   	 FROM 
						  	   	 	 ".MF_TABLE_PREFIX."element_options 
						  	   	WHERE 
				...
				foreach($matrix_element_id_pair as $source_element_id=>$dest_element_id){
					$query = "INSERT INTO 
										 ".MF_TABLE_PREFIX."element_options (`form_id`,`element_id`,`option_id`,`position`,`option`,`option_is_default`,`option_is_hidden`,`live`)
							  	   SELECT 
							  	   		 `form_id`, '{$dest_element_id}' as new_element_id,`option_id`,`position`,`option`,`option_is_default`,`option_is_hidden`,'2' as `live` 
							  	   	 FROM 
							  	   	 	 ".MF_TABLE_PREFIX."element_options 
							  	   	WHERE 
							  	   		 form_id = ? AND element_id = ? AND (live=1 OR live=2);"; 
	...
	if(in_array($element_type,array('select','radio','checkbox'))){
		...
		if(($action == 'click_new') || ($action == 'drag_new')){
			$el_options_json =<<<EOT
			{"options"		:	[{"option" : "First option", "is_default":0, "is_hidden":0, "is_db_live":"0", "id": 1},
							 	 {"option" : "Second option", "is_default":0, "is_hidden":0, "is_db_live":"0","id": 2},
							 	 {"option" : "Third option", "is_default":0, "is_hidden":0, "is_db_live":"0", "id": 3}]
			}
EOT;
	...
	if($element_type == 'matrix'){
		...
		if(($action == 'click_new') || ($action == 'drag_new')){
			$el_options_json =<<<EOT
			{"options"		:	[{"option" : "Answer A", "is_default":0, "is_hidden":0, "is_db_live":0, "id": 1},
							 	 {"option" : "Answer B", "is_default":0, "is_hidden":0, "is_db_live":0, "id": 2},
							 	 {"option" : "Answer C", "is_default":0, "is_hidden":0, "is_db_live":0, "id": 3},
							 	 {"option" : "Answer D", "is_default":0, "is_hidden":0, "is_db_live":0, "id": 4}]
			}
EOT;
			...
			foreach ($column_data as $option_id=>$column_value){
				...
				$el_options_array[$i]->is_hidden  = 0;
	...
	if(in_array($element_type,array('select','radio','checkbox'))){
		if(($action == 'click_new') || ($action == 'drag_new')){
			...
			$element_properties['options'][1]['is_hidden'] = '0';
			...
			$element_properties['options'][2]['is_hidden'] = '0';
			...
			$element_properties['options'][3]['is_hidden'] = '0';
