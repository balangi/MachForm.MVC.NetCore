 ...
 ********************************************************************************/
	...
	if(mf_is_form_submitted()){ //if form submitted
			...
			if($submit_result['status'] === true){
				if(!empty($submit_result['form_resume_url'])){ //the user saving a form, display success page with the resume URL
					...
					if(is_numeric($target_page_id)){
						header("Location: ".$_SERVER['PHP_SELF']."?id={$input_array['form_id']}&mf_page={$target_page_id}");
						exit;
						...
						if(in_array($form_properties['payment_merchant_type'], array('stripe','authorizenet','paypal_rest','braintree'))){
							...
							header("Location: ".mf_get_dirname($_SERVER['PHP_SELF'])."/payment.php?id={$input_array['form_id']}");
							exit;
						...
						header("Location: ".mf_get_dirname($_SERVER['PHP_SELF'])."/confirm.php?id={$input_array['form_id']}{$page_num_params}");
						exit;
					...
					if(!empty($submit_result['next_page_number'])){ //redirect to the next page number
						...
						header("Location: ".$_SERVER['PHP_SELF']."?id={$input_array['form_id']}&mf_page={$submit_result['next_page_number']}");
						exit;
		...
		$form_id 		= isset($_GET['id']) ? (int) trim($_GET['id']) : 0;
		$page_number	= isset($_GET['mf_page']) ? (int) trim($_GET['mf_page']) : 0;
		
		$page_number 	= mf_verify_page_access($form_id,$page_number);
		
		$resume_key		= isset($_GET['mf_resume']) ? trim($_GET['mf_resume']) : false;
		if(!empty($resume_key)){
		...
		$edit_key		= isset($_GET['mf_edit']) ? trim($_GET['mf_edit']) : false;
		if(!empty($edit_key)){
