 ...
 ********************************************************************************/
	...
	if(!empty($_FILES) && !empty($_POST['session_id'])){
		...
		if(!file_exists($machform_data_path.$mf_settings['data_dir']."/themes/images")){
			$old_mask = umask(0);
			mkdir($machform_data_path.$mf_settings['data_dir']."/themes/images",0755);
			umask($old_mask);
		}
		...
		if(move_uploaded_file($source_file,$destination_file)){
			...
			if(MF_STORE_FILES_AS_BLOB === true){
				mf_ap_form_themes_files_insert($dbh,$form_id,$destination_file);

				$file_name_only = pathinfo($destination_file,PATHINFO_BASENAME);
				$uploaded_file_url = $mf_settings['base_url'].'theme_img.php?q='.base64_encode("file_name=".$file_name_only);
			}
		}else{
			$upload_success = false;
